...
// 🟢 GREEN Phase: Phase 1 Implementation - Vitest Mock Fixes

import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest'
import type { DebugAnalysis, DebugPlan, DebugSession } from '../lib/types/memory'

// Mock using factory functions as per Context7 guidance for proper Vitest integration
// Only mock what we actually need to avoid breaking other imports
vi.mock('fs/promises', () => ({
  readFile: vi.fn(),
}))
...
describe('Debug Assistant', () => {
  ...
  describe('Active Investigation', () => {
    it('should perform comprehensive code investigation', async () => {
      ...
      const { readFile } = await import('fs/promises')
      ;(readFile as any).mockResolvedValue('export const logger = console.log')
    ...
    it('should check environment configurations', async () => {
      ...
      ;(readFileEnv as any).mockResolvedValue(envContent)
